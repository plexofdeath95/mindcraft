...
export class Agent {
  ...
  async _setupEventHandlers(save_data, init_message) {
    ...
    if (save_data?.self_prompt) {
      ...
      await this.handleMessage(
        "system",
        `You have restarted and this message is auto-generated. Continue the conversation with ${this.last_sender}`
      );
  ...
  async handleMessage(source, message, max_responses = null) {
    ...
    if (!self_prompt) this.last_sender = source;
    ...
    const checkInterrupt = () =>
      this.self_prompter.shouldInterrupt(self_prompt) || this.shut_up || responseScheduledFor(source);
  ...
  async routeResponse(to_player, message, translate_up_to = -1) {
    ...
    if (self_prompt) this.bot.chat(message);
    //else
