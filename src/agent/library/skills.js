...
export async function craftRecipe(bot, itemName, num = 1) {
  ...
  if (craftLimit.num < num)
    log(
      bot,
      `Not enough ${craftLimit.limitingResource} to craft ${num}, crafted ${craftLimit.num}. You now have ${
...
export async function attackNearest(bot, mobType, kill = true) {
  ...
  if (players.length > 0) {
    ...
    // If a closest player is found and within range, attack
    if (closestPlayer && closestDistance <= 200) {
      await attackEntity(bot, closestPlayer, kill);
      return true;
    }
  }
...
export async function placeBlock(bot, blockType, x, y, z, placeOn = "bottom", dontCheat = false) {
  ...
  if (
    ...
    (pos.distanceTo(targetBlock.position) < 1 || pos_above.distanceTo(targetBlock.position) < 1)
  ) {
    // too close
...
export async function followPlayer(bot, username, distance = 4) {
  ...
  while (!bot.interrupt_code) {
    ...
    if (bot.modes.isOn("unstuck")) {
      ...
      if (is_nearby) bot.modes.pause("unstuck");
      else bot.modes.unpause("unstuck");
    }
  }
  return true;
}
...
export async function activateNearestBlock(bot, type) {
  ...
  log(
    bot,
    `Activated ${type} at x:${block.position.x.toFixed(1)}, y:${block.position.y.toFixed(
      1
